...
def split_batches(num_local_devices, halo_batch, omega_m_batch, tpcfs_batch):
    ...
    omega_m_batch = jax.tree_map(lambda x: np.split(x, num_local_devices, axis=0), omega_m_batch)
    halo_batch, omega_m_batch = np.array(halo_batch), np.array(omega_m_batch)

    if tpcfs_batch is not None:
        tpcfs_batch = jax.tree_map(lambda x: np.split(x, num_local_devices, axis=0), tpcfs_batch)
