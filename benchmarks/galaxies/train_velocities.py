...
def get_iterators(
    ...
    dataset, _, _, std, _, _ = get_halo_dataset(
        ...
        num_samples=args.training_set_size,
        split="train",
...
def eval_step(pstate, x_batch_masked, x_batch, mask,):
...
def get_loss_for_iterator(iterator,  pstate, key, num_local_devices):
    ...
    for x_batch, _ in iterator:
        x_batch_masked, x_batch, mask = mask_batch(
            x_batch,
            key,
...
def train_model(
    ...
    test_loss = get_loss_for_iterator(
        iterators['test'], best_state, key, num_local_devices,
    )
...
if __name__ == "__main__":
    ...
    output_path = Path('/global/u1/c/cuesta/eqnn-jax/benchmarks/galaxies/velocity_task/')
    ...
    # LOAD DATA
    iterators, std = get_iterators(batch_size=batch_size,)
    ...
    train_model(
        ...
        num_local_devices=num_local_devices,
        save_model=True,
        ckpt_dir= output_path / f"gnn_{args.training_set_size}"
    )
    iterators, std = get_iterators(batch_size=batch_size,)
    train_model(
        iterators,
        ...
        num_local_devices=num_local_devices,
        save_model=True,
        ckpt_dir= output_path / f"segnn_{args.training_set_size}",
    )
    # iterators, std = get_iterators(batch_size=batch_size,)
    # train_model(
    #     iterators,
